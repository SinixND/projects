...
// Finally, the way to react to such events must be done in "handlers". In our sample, we react to three menu items, one for our custom menu command and two for the standard "Exit" and "About" commands (any program should normally implement the latter two). Notice that these handlers don't need to be neither virtual nor public.
...
bool MyApp::OnInit() {
    MyFrame *frame = new MyFrame("Hello World", wxPoint(50, 50), wxSize(450, 430) );
    frame->Show(true);
    return true;
}
...
MyFrame::MyFrame(const wxString& title, const wxPoint& pos, const wxSize& size) : wxFrame(NULL, wxID_ANY, title, pos, size) {
    ...
    CreateStatusBar();
    SetStatusText("Welcome to my first window!");
}
...
void MyFrame::OnAbout(wxCommandEvent& event) {
    wxMessageBox("This is my about information!(Comma), second argument (vertical slash)| wxICON_INFFORMATIOIN");
}
...
void MyFrame::OnHello(wxCommandEvent& event) {
    wxLogMessage("Hello world log message!");
}